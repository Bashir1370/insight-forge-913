-- Finalize the database-backed Resource Tour editor.
-- This migration is safe to run after the earlier resource_tours migrations.

alter table public.resource_hotspots
  add column if not exists hotspot_key text;

-- Preserve any existing GDC rows by assigning the semantic hotspot key from step order.
...
-- Reuse the same updated_at behavior as the rest of the CMS.
...
-- Replace the permissive authenticated policies from the initial prototype.
...
-- Seed the GDC editor without overwriting values an admin may already have saved.
