...
export const Route = createFileRoute("/admin_/resource-tours")({
  ...
  beforeLoad: async () => {
    ...
    const { data: role } = await (supabase as any).from("user_roles").select("role").eq("user_id", user.id).eq("role", "admin").maybeSingle();
    if (!role) throw redirect({ to: "/dashboard" });
  },
  component: ResourceTourAdmin,
});

async function saveResourceImage(imageUrl: string) {
  await supabase.from("resource_tours").upsert({ slug: "gdc", title: "GDC Resource Tour", image_url: imageUrl }, { onConflict: "slug" });
}

async function saveHotspots(items: typeof fallbackHotspots) {
  const { data: resource } = await supabase.from("resource_tours").upsert({ slug: "gdc", title: "GDC Resource Tour", image_url: gdcImage }, { onConflict: "slug" }).select("id").single();
  if (!resource) return;
  await supabase.from("resource_hotspots").delete().eq("resource_id", resource.id);
  await supabase.from("resource_hotspots").insert(items.map((item) => ({ resource_id: resource.id, step: Number(item.id), title: item.title, x: item.x, y: item.y, width: item.width, height: item.height })));
}
...
async function saveContent(items: typeof fallbackContent) {
  ...
  await supabase.from("resource_content_blocks").insert(items.map((item) => ({ resource_id: resource.id, key: item.key, label: item.label, value: item.value })));
}

function ResourceTourAdmin() {
  const [hotspots, setHotspots] = useState(fallbackHotspots);
  const [content, setContent] = useState(fallbackContent);
  const [imageUrl, setImageUrl] = useState(gdcImage);

  useEffect(() => {
    async function load() {
      const { data: resource } = await supabase.from("resource_tours").select("id,image_url").eq("slug", "gdc").maybeSingle();
      if (!resource) return;
      if (resource.image_url) setImageUrl(resource.image_url);
  ...
  return <main className="min-h-screen bg-slate-50 p-8" dir="rtl">
    ...
    <div className="mt-6 space-y-6">
      <VisualAssetEditor imageUrl={imageUrl} onSave={(url) => { setImageUrl(url); saveResourceImage(url); }} />
      <VisualPageEditor title="GDC Resource Tour" imageUrl={imageUrl} hotspots={hotspots} onSave={(items) => { setHotspots(items); saveHotspots(items); }} />
