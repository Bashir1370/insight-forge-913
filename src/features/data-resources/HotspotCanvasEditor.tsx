...
export function HotspotCanvasEditor({
  ...
  function move(e: React.PointerEvent) {
    if (dragRef.current) {
      ...
      setItems((current) => current.map((item) => item.id === dragRef.current?.id ? { ...item, x: Math.max(0, Math.min(100-item.width, item.x+dx)), y: Math.max(0, Math.min(100-item.height, item.y+dy)) } : item));
    ...
    if (resizeRef.current) {
      ...
      setItems((current) => current.map((item) => item.id === resizeRef.current?.id ? { ...item, width: Math.max(3, Math.min(80,item.width+dw)), height: Math.max(3, Math.min(80,item.height+dh)) } : item));
  ...
  return (
    <div className="rounded-3xl border bg-white p-5">
      <div className="flex items-center justify-between">
        ...
        <button className="rounded-xl bg-teal-600 px-4 py-2 text-white font-bold" onClick={() => onSave?.(items)}>ذخیره تغییرات</button>
      </div>
      <div className="relative mt-4 aspect-video overflow-hidden rounded-2xl bg-slate-100" onPointerMove={move} onPointerUp={() => {dragRef.current=null;resizeRef.current=null;}}>
        {imageUrl && <img src={imageUrl} className="absolute inset-0 h-full w-full object-contain" />}
        {items.map((item)=>(
          <button key={item.id} onPointerDown={(e)=>startDrag(item.id,e)} className={`absolute border-2 border-dashed border-teal-500 bg-teal-500/10 text-xs font-bold cursor-move ${selected===item.id?"ring-4 ring-teal-300":""}`} style={{left:`${item.x}%`,top:`${item.y}%`,width:`${item.width}%`,height:`${item.height}%`}}>
            {item.title}<span onPointerDown={(e)=>startResize(item.id,e)} className="absolute right-0 bottom-0 h-3 w-3 cursor-se-resize bg-teal-500" />
          </button>
        ))}
      </div>
    </div>
