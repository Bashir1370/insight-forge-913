        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
            ...
            <div className="p-6 sm:p-8">
              ...
              {section.insight && (
                <div className="mt-6 flex items-start gap-3 rounded-2xl border border-teal-200 bg-teal-50 p-5">
                  ...
                  <div className="text-sm leading-8 text-teal-950">{section.insight}</div>
                </div>
              )}
          ...
          <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {sectionIndex < sections.length - 1 && (
              ...
              >
                {isCorrect ? "ادامه مسیر مفهومی" : "برای ادامه، پاسخ درست را پیدا کنید"}
  ...
  return (
    <div
      className={[
        ...
        emphasized ? "border-teal-300 bg-teal-50" : "border-slate-200 bg-slate-50",
      ].join(" ")}
    >
            ...
            >
              ...
              {open && (
                <div className="mt-4 border-t border-violet-100 pt-4">
                  <p className="text-sm leading-8 text-slate-700">{term.explanation}</p>
                  {term.example && (
...
function ScenarioCard({ scenario }: { scenario: GuidedScenario }) {
  return (
    <section className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-5 sm:p-6">
      ...
      <h3 className="mt-2 text-lg font-black text-amber-950">{scenario.title}</h3>
      <p className="mt-2 text-sm leading-8 text-amber-950/80">{scenario.description}</p>
      {scenario.items && (
        <div className="mt-4 grid gap-2">
          {scenario.items.map((item) => (
            <div key={item} className="rounded-xl border border-amber-200 bg-white/70 px-4 py-3 text-sm leading-7 text-slate-700">
              {item}
            </div>
          ))}
        </div>
    ...
    <section className="mt-7 rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 font-bold leading-8 text-slate-950">{question.question}</p>
        </div>
      </div>
      ...
      {answered && (
        ...
        >
          <p className={correct ? "text-sm font-black text-emerald-900" : "text-sm font-black text-amber-950"}>
...
function BridgeCard({ bridge, unlocked }: { bridge: ConceptBridge; unlocked: boolean }) {
  return (
    <section
      className={[
        "mt-6 rounded-3xl border p-5 sm:p-6",
        unlocked ? "border-sky-200 bg-sky-50" : "border-slate-200 bg-slate-50",
      ].join(" ")}
    >
      <p className={unlocked ? "text-xs font-black text-sky-700" : "text-xs font-black text-slate-400"}>
        چرا باید بخش بعد را بخوانم؟
      </p>
      <p className={unlocked ? "mt-2 text-base font-black leading-8 text-sky-950" : "mt-2 text-base font-black leading-8 text-slate-400"}>
        {bridge.openQuestion}
      </p>
      <p className={unlocked ? "mt-2 text-sm leading-7 text-sky-900" : "mt-2 text-sm leading-7 text-slate-400"}>
        {unlocked
          ? bridge.nextStep
